...
from emp e , emp p
where e.job = p.job and p.empno = '7788';
--------------------------------------------------
select ename, job 
from emp
...
from emp e
where e.sal = (select min(sal) from emp c where c.deptno = e.deptno );
...
select e.ename
from emp e  
where empno not IN (select distinct mgr from emp c where e.empno = c.mgr);

-- 50. 부하직원이 있는 사원의 이름을 표시하시오.
select e.ename
from emp e
where empno in (select mgr from emp m where e.empno = m.mgr) ;
