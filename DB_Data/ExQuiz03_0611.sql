...
SELECT ENAME, TO_CHAR(HIREDATE, 'YY') , TO_CHAR(HIREDATE, 'MM') , TO_CHAR(HIREDATE, 'DY')
FROM EMP;
...
SELECT JOB, DEPTNO,
       DECODE(DEPTNO, 10, SUM(SAL)) AS "10",
       DECODE(DEPTNO, 20, SUM(SAL)) AS "20",
       DECODE(DEPTNO, 30, SUM(SAL)) AS "30",
       SUM(SAL)
FROM EMP
GROUP BY JOB, DEPTNO
ORDER BY DEPTNO;