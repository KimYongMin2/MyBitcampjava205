...
select sum(saleprice) as Total_SaleBook_Price
from orders
where custid = 1
;
-- (4) 박지성이구매한도서의수(박지성의고객번호는1번으로놓고작성)
select count(custid) as Total_Salebook_Num
from orders
where custid = 1
;
...
-- (1) 마당서점도서의총개수
select count(bookid) as Total_book_Num
from book
;
-- (2) 마당서점에도서를출고하는출판사의총개수
select count(DISTINCT publisher) as Total_publisher_Num
from book
;
-- (3) 모든고객의이름, 주소
select name, address
